...
export function WholesaleForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/wholesale-contact', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ companyName, city, email, message }),
      });
      ...
      trackLead({ email, firstName: companyName });
      setStatus('success');
      setCompanyName('');
      setCity('');
      setEmail('');
  ...
  if (status === 'success') {
    return (
      <div className="flex flex-col items-center justify-center min-h-105 text-center">
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <p className="text-xs text-gray-700">
        ...
        <Link href="/podminky" className="underline hover:text-gray-900">
