...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const [productsRes, categories, infoPages] = await Promise.all([
    getProducts({ pageSize: 200 }).catch(() => ({ data: [] as { slug: string; updatedAt: string }[] })),
    getCategories().catch(() => []),
    getInfoPages().catch(() => []),
  ]);
  ...
  const staticPages: MetadataRoute.Sitemap = [
    ...
    { url: `${SITE_URL}/produkty`, lastModified: new Date(), changeFrequency: 'daily', priority: 0.9 },
